...
- **Two doors write the row, and the grant is the first move (the pick round, plan §7, 2026-09-20).** `/connect/github/start?workspace&channel&actor` seals the state (the social flows' idiom) and sends the person to GitHub's install page, whatever the project holds: the page is where they pick the account and the repositories, once. The callback records the installation with its `selection` and the workspace, then resolves the room with `granted`: the project's GitHub-addressed repository readable → the row; the project naming none (or a folder attached from a desktop) and the App reading exactly one repository → **attached to the project** (`repos` + `project_repos`, primary when the project has no primary, the `repo.linked` event) and the row; several → the page counts them and sends the person to the pick. `POST /v1/github/resolve { channel, repo? }` (HUMAN_ONLY, in the workspace; the workspace comes from the room, so a project with no repository resolves too) answers from what the App reads NOW for the workspace: the installations remembered for it, refreshed installation by installation (`readableRepos`, a dead one forgotten), as `repos` with a `hint` (the one named like the project's folder). With `repo` it is the pick: verified, attached, connected in one move. An open step never attaches on its own: only the grant's callback and the pick do. The step (`GitHubStep`, one state machine `useGitHubGrant` for the popover, setup step 5 and the dependency card) shows the pick when the answer lists repositories, the grant otherwise, polls every five seconds while GitHub is open, and offers **Check again**. Before the round the step demanded a repository with a GitHub address before the grant could start, so a desktop-born project (its folder `local/<name>`) ended on "Attach a repository first" with no door (George's screenshot, 2026-09-20). The public announce door keeps its slug-state branch on the same two routes (`github-connect.ts`, the resolver in `github-resolve.ts`).
- **The reads go through the API, like X.** `GET /v1/repo/changes|file|tree?channel=` (`github-connect.ts`): the actor in the workspace, the room's project's repository with a GitHub address (`store.announcements.repoForChannel`: the primary among those, so a local folder as the primary never blocks a read), the `connected` row (else `NOT_CONNECTED`), the installation (table, else GitHub's own answer, `selection = 'all'` matched by account), one installation token per installation per hour, in memory only. A deleted installation answers 404 on the mint: the row turns `reauth_required`, the read says `RECONNECT_REQUIRED`, the attention bar lights, and a second read never touches GitHub. Files: 60 KB of text, binaries and directories refused by name, 1 MB is the ceiling. Trees: 500 entries, filtered by path.
...
Proven live for the pick round (2026-09-20, the dev stack, `docs/design/github-connector-2026-09/evidence/live-pick-*`): a project with a folder attached from a desktop opened the step on the grant face with the folder's name as the hint, the door answered 302 to GitHub's install page with the sealed state, the poll flipped the step to the pick once the installation named the workspace, Connect attached the real private repository as a second repository beside the folder and wrote the row, and `/v1/repo/changes` + `/file` read it through the App. Plan §7.7 has the walk and the two findings.
