...
export const CommandSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('content.revise'), item: z.string().min(1), body: z.string().trim().min(1).max(10_000).optional(), imageBrief: z.string().trim().max(2000).optional(), script: z.string().trim().min(1).max(10_000).optional(),
    ...
    videoMeta: z.object({ tier: z.string().max(40), model: z.string().max(80), seconds: z.number().int().min(1).max(60), credits: z.number().int().min(0), at: z.string().datetime() }).optional(),
