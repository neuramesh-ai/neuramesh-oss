...
export function makeOrchTurn(ctx: {
  ...
  - CREATOR VIDEO (the UGC playbook, "ugc scripts", "creator videos") is TWO turns: read the shelf, then **propose_angles** (the angle card: two to five angles from the product's facts, the platforms and the film's length as chips) and STOP. The human's pick wakes you; only then draft_posts, one video post per picked platform in that angle, the script written to the picked length. A script drafted before the pick is refused.
